        ...
        for (var i = 0; i < charge_array.length; i++) {
            ...
            if (current_element.point_or_line == 1) {
                ...
                if (current_element.polarity == 1) {
                    point_charge_gradient.addColorStop(0, 'rgba(8,143,143)');
                    point_charge_gradient.addColorStop(0.8, 'rgba(200,200,200,.9)');
                    point_charge_gradient.addColorStop(1, 'rgba(255,255,255,0)');
                    ctx.fillStyle = "black";
                }
                else {
                    point_charge_gradient.addColorStop(0, ('rgba(255,0,0)'));
                    point_charge_gradient.addColorStop(0.8, ('rgba(255,165,0)'));
                    point_charge_gradient.addColorStop(1, 'rgba(255,255,255,0)');
                    ctx.fillStyle = "blue";
                }
            ...
            for (var i = 0; i < 24; i++) {
                ...
                if (draw_field_line) {
                    if (x1 < 0.01 && x1 > -0.01) {
                        ...
                        slope = y1 / x1;
                        x_displacement = 7 * (Math.sqrt(1 / ((slope * slope) + 1))); // ensures every line is the same length
                        y_displacement = 0;
                    }
